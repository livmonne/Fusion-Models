...
Demo Pairs (input/output grids)                Test Input Grid
  ...
  logits + repr  logits + repr  logits + repr
       ...
       └──────────────┘
                      │
     Blended logits → reshape to (batch, H×W, 10)
     Per-cell colour predictions for the output grid
```
...
1. **Perceive** — grid cells are embedded with positional and type
   ...
   demos to the test input.

2. **Pool** — the cross-attended test tokens are mean-pooled (masking
   padding) into a single shared embedding `h`.

3. **Specialise** — three expert pathways process `h` independently:
   - **RuleMemory** retrieves and applies stored rules.
   - **RuleGenerator** synthesises one-shot rules on the fly.
   - **GuessComponent** uses self-attention for fuzzy pattern matching.

4. **Arbitrate** — the DecisionRouter uses multi-head cross-attention to
   decide how much to trust each pathway.  The final prediction is a soft
   mixture weighted by the routing coefficients.
